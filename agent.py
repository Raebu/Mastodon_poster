...
def main():
 m=Mastodon();me=m.me();print("Authenticated Mastodon account:",me.get("acct"));seen=set();actions=0;run_counts=Counter()
 ...
 for item in items:
  ...
  if not allowed:
   print("NO_ACTION",sid,acct,"SOCIAL_OS",reason);social_os.log_interaction(acct,sid,"NO_ACTION",reason,DRY);continue
  choice=decide(text);print(choice,sid,acct)
  ...
  actions+=1;run_counts[acct.lower()]+=1;social_os.log_interaction(acct,sid,choice,"selected by engagement gate",DRY)
 social_os.log_run("engagement",actions,DRY,f"unique_accounts={len(run_counts)}")
 print(f"SUMMARY actions={actions} unique_accounts={len(run_counts)} dry_run={DRY}")
